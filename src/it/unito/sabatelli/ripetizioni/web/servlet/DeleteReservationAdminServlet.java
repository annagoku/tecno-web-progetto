...
@WebServlet(name="DeleteReservationAdmin", urlPatterns = "/servlets/DeleteReservationAdmin")
    ...
    private void processRequestPost (HttpServletRequest request, HttpServletResponse response) throws javax.servlet.ServletException, IOException {
      ...
      try{
        if(!user.getRole().equalsIgnoreCase("administrator")){
          ...
          gresp.setErrorOccurred("Operazione non autorizzata");
          response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
          response.getWriter().write(gson.toJson(gresp));
          return;
        }else{
          Dao dao = (Dao) request.getServletContext().getAttribute(Dao.DAONAME);
          Lesson l=gson.fromJson(request.getParameter("reservationToDelete"), Lesson.class);
          if (l!=null){
            int row=dao.deleteReservationAdmin(l);
